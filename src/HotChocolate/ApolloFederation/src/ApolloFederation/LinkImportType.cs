...
public class LinkImportType: ScalarType<LinkImport>
{
    public LinkImportType() : base(WellKnownTypeNames.Import)
    {
    ...
    public override object? ParseLiteral(IValueNode literal)
        => literal switch
        {
            StringValueNode s => new LinkImport(s.Value),
    ...
    public override IValueNode ParseValue(object? runtimeValue)
        ...
        {
            null => NullValueNode.Default,
            LinkImport i => i.ToValueNode(),
            _ => throw new SerializationException(TypeResourceHelper.Scalar_Cannot_ParseValue(Name, runtimeValue.GetType()), this)
        };

    private bool TryParseObject(ObjectValueNode node, [NotNullWhen(true)] out LinkImport? import)
    {
